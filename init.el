...
(defun kwarks/config-el-complete-p (el-file)
  "Return non-nil when EL-FILE looks like a full tangled config.
Incomplete tangling (e.g. config.org writing then deleting `config.el')
can leave a file that passes mtime checks but omits the Package Management
section, so `use-package' byte-compiles to bare `require' forms."
  (with-temp-buffer
    (insert-file-contents el-file)
    (and (re-search-forward "use-package-always-ensure" nil t)
         (re-search-forward "(provide 'config)" nil t))))
...
is newer than config.elc."
  (let* ((dir user-emacs-directory)
    ...
    (and (file-exists-p el-file)
         (kwarks/config-el-complete-p el-file)
...
Write the aggregate to a temp file first: config.org's `:tangle yes' targets
`config.el', so tangling it while `with-temp-file' already owns `config.el'
can truncate or delete the output mid-build."
  (require 'org)
  (let* ((dir user-emacs-directory)
         (out (expand-file-name "config.el" dir))
         (files (kwarks/config-source-files))
         (tmp-out (make-temp-file "kwarks-config-" nil ".el")))
    (unwind-protect
        (with-temp-file tmp-out
          ...
          (dolist (f files)
            ...
            (dolist (tangled (org-babel-tangle-file f))
              (let ((tangled (expand-file-name tangled)))
                (insert-file-contents tangled)
                (goto-char (point-max))
                (unless (equal tangled (expand-file-name out))
                  (delete-file tangled)))))
          (insert "\n(provide 'config)\n;;; config.el ends here\n"))
      (rename-file tmp-out out t))
    (unless (kwarks/config-el-complete-p out)
      (error "Tangled %s is incomplete (missing use-package-always-ensure)" out))
